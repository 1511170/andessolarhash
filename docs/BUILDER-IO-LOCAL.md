...
El comando `code` de Builder.io requiere una **terminal TTY** (interactiva). No puede ejecutarse en entornos sin TTY (por ejemplo desde el agente de Cursor).
...
Ejecutar en la raíz del proyecto desde una terminal con TTY (PowerShell, CMD, Git Bash con winpty si aplica). La salida (HTML/React) se puede pegar y convertir a Svelte según el plan en `.cursor/plans/`.
